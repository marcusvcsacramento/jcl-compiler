...
with open('arquivos/DIARIA', 'rt') as f:
    ...
    for linha in f.read().split('\n'):
        ...
        job_temp=sub_string(job,'ANO_DATA_MOVIMENTO_ANO',ano_data_movimento_ano)
        job_temp=sub_string(job,'ANOMES_MOVIMENTO',anomes_movimento)
        job_temp=sub_string(job,'MES_MOVIMENTO',mes_movimento)
        job_temp=sub_string(job,'DIA_MOVIMENTO',dia_movimento)
        job_temp=sub_string(job,'MESDIA_MOVIMENTO',mes_movimento+dia_movimento)
        job_temp=sub_string(job,'DATA_ANTERIOR',data_anterior)
        job_temp=sub_string(job,'ULTIMO_DIA_DO_MES_ANTERIOR',ultimo_dia_do_mes_anterior)
        job_temp=sub_string(job,'ULTIMO_DIA_DO_MES',ultimo_dia_do_mes)
        job_temp=sub_string(job,'HORA_MINUTO',hora_minuto)
