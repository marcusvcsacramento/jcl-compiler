...
arquivolog = 'arquivos/log/'+datetime.datetime.now().strftime("%Y-%m-%d_%H-%M")+'.'+sistema+'.'+ambiente+'.TESTE.log'
...
with open('arquivos/TESTE', 'rt') as f:
    ...
    for linha in f.read().split('\n'):
        ...
        sub_string_file(job_modelo,'arquivos/cartao/'+sistema+'/'+ambiente+'/JOBJCL.tmp','DATA_MOVIMENTO',data_movimento)
        ...
        sub_string_file('arquivos/cartao/'+sistema+'/'+ambiente+'/JOBJCLI.tmp',job_final,'ID_EXECUTOR','DES_SP01')
