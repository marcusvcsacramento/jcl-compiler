...
try:
    ...
    lib_validacao = config.get(sistema+'.'+ambiente,'lib_validacao')
...
with open(arquivo_jobs,'r') as job:
    for linha in job.read().split('\n'):
        ...
        load_value=''
        try:
            print(lib_validacao+data_execucao+'.'+job_name+'.'+jobnumber)
            ...
            with open(tom_directory+'/'+programa+'-'+jobnumber,'r', encoding="latin-1") as arquivo:
                for linha in arquivo.read().split('\n'):
                    if len(linha)==0:
                        continue
                    validacao = linha.split(';')
                    max_rc=validacao[1]+';'+validacao[3]
            try:
                ftp_load = ftplib.FTP()
            ...
            if max_rc:
                if max_rc.find("OK"):
                    color="\033[41m"
                print(color+"{};{};{};{}\033[0m\n".format(programa,jobnumber,max_rc,load_value))
                saida.write("{};{};{};{}\n".format(programa,jobnumber,max_rc,load_value))
            saida.flush()
        except:
