...
def verifica_retorno(programa,jobnumber,sistema,ambiente):
    job_retorno=True
    try:
    ...
    load_value=''
    try:
        print("######   Realizando GET do JOB "+sistema.center(8)+" "+ambiente.center(8)+": "+programa.center(8)+"    ######")
        ...
        with open(tom_directory+'/'+programa+'-'+jobnumber,'r', encoding="latin-1") as arquivo:
            for linha in arquivo.read().split('\n'):
                ...
                if max_rc:
                    job_retorno=False
                    color="\033[42m"
                    if str(max_rc.group(1))!="0000" and str(max_rc.group(1))!="0002" and str(max_rc.group(1))!="0004":
                            color="\033[42m"
                            job_retorno=True
